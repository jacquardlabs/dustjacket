...
| Format | Opens with | Emoji | Length | Best for | Example |
|--------|------------|-------|--------|----------|---------|
| `minimal` | title + one-liner | none | shortest | scripts, small utilities | [rendered](docs/examples/tokenbucket-minimal.md) |
| `standard` | title + install | none | lean | most libraries, CLIs, plugins | [rendered](docs/examples/tokenbucket-standard.md) |
| `reference` | a table of contents | none | long | tools whose README is the docs | [rendered](docs/examples/tokenbucket-reference.md) |
| `tutorial` | "Getting started" | light, structural | medium-long | onboarding-focused projects | [rendered](docs/examples/tokenbucket-tutorial.md) |
| `landing` | a hero image | structural | medium | products, frameworks | [rendered](docs/examples/tokenbucket-landing.md) |

Each row's **rendered** link opens the same `tokenbucket` library written in that format, shown as GitHub renders it. Voice is layered on top and changes wording only, never the layout.
