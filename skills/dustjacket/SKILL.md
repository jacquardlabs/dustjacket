...
- An existing README is messy, generic, or reads like AI boilerplate → **prettify** (the v1 mode).
- A repo has no README, or a stub → **generate** (not yet built; see milestones).
- A README may no longer match the code → **check** (not yet built; see milestones).
...
| Mode | Job | Status |
|------|-----|--------|
| `prettify` | Restyle an existing README to its repo-type house style and a chosen voice | **available** |
| `check` | Read-only drift gate; fails when the README no longer matches reality | planned (M3) |
| `generate` | Build a README from repo signals, interviewing for gaps | planned (M4) |

If asked for `check` or `generate`, say they're planned and offer `prettify` on the existing README.
...
7. **Flag the gaps.** Any required section with no real source becomes a `> TODO:` note, never fiction.
8. **Show a diff and get approval before writing.** Apply the output contract in `references/output.md`.
